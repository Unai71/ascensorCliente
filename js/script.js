...
//import Paqueteria from './listaPaquetes.js';
document.addEventListener('DOMContentLoaded', () => {
  ...
  //  const edificio = new Edificio(ascensor);
    const listaPendientes = document.getElementById('paquetes-pendientes');
    const listaEnTransito = document.getElementById('lista-paquetes');
    ...
    function manejarAgregarPaquete(event) {
        ...
        //**He añadido la lista paquetes pendientes para meter los paquetes ahí, luego si está en pb los recoge */
        ...
        listaPendientes.appendChild(elementoPaquete);
    ...
    function actualizarEstadoAscensor() {
        ...
        //! Esto igual toca moverlo de sitio (?)
        if (ascensor.pisoActual === 0) {
            console.log(typeof listaPendientes)
            for(const paquete of listaPendientes)
            {
                console.log("hay paquete");
                // Asegúrate de que esta función actualiza el peso total en el ascensor.
                if ((ascensor.cargaActual + paquete.peso) <= ascensor.cargaMaxima) {
                    ascensor.cargar(paquete);
                    paqueteria.en_transito.appendChild(paquete);
                }
            }
        }
